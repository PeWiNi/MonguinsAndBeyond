...
public class Trap_Spikes : Trap
{
    ...
    void OnTriggerStay(Collider col)
    {
        if (col.tag == "Player")
        {
            col.transform.GetComponent<PlayerBehaviour>().TakeDamageWhileMoving(ticDamage, owner);
        }
    }
    ...
    void OnTriggerEnter(Collider col)
    {
        if (col.tag == "Player")
        {
            ...
            col.transform.GetComponent<PlayerStats>().TakeDmg(col.transform.GetComponent<PlayerStats>().maxHealth * impactDamage, owner);
        }
    }
